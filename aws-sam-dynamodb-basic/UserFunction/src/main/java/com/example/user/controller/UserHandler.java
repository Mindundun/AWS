...
public class UserHandler implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    private static final ObjectMapper objectMapper = new ObjectMapper();  // JSON to DTO

    // 생성자에서 의존성 직접 조립 (@Autowired가 없으니)
    public UserHandler() {
        // DB 클라이언트 생성
        DynamoDbClient ddb = DynamoDbClient.builder()
                .httpClient(UrlConnectionHttpClient.create())  // Cold Start 완화 (무거운 Netty 대신 가벼운 URLConnection 클라이언트 사용)
