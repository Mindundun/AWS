package com.example.awsApiGatewayBasic;
...
public class WelcomeHandler implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent input, Context context) {
        ...
        // input.getQueryStringParameters()는 쿼리스트링이 없으면 null일 수 있으므로 주의해야 함
        ...
        // HTTP 헤더는 대소문자를 구분하지 않는 경우가 많지만, Lambda 이벤트에서는 넘어온 그대로 확인 필요
        ...
        if ("ko".equalsIgnoreCase(lang)) {
            ...
            message = String.format("Bonjour, %s! User-Agent: %s", name, userAgent);
        } else {
            message = String.format("Hello, %s! Your client is %s.", name, userAgent);
        }
